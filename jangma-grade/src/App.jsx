...
function formatPriceLimit(dealType, value) {
  if (dealType === '전세') {
    return value >= 10000 ? `${(value / 10000).toFixed(value % 10000 === 0 ? 0 : 1)}억 이하` : `${value.toLocaleString()}만원 이하`;
  }
  ...
  return (
    ...
    >
      ...
      {(dealType === '전체' || dealType === '전세') && (
        <span className="priceChip">전세 {formatPriceLimit('전세', priceLimits.전세).replace(' 이하', '')}</span>
      )}
    </span>
  );
}
...
function coachItems(property) {
  const items = [
    ['Q1', '2020년 7월, 이 집에 물이 들어왔었나요? (거짓말하기 어려운 직접 질문)'],
    ['Q2', '벽·창문 아래 물때 자국이나 곰팡이 흔적이 있는지 확인하세요.'],
  ];

  if (property.banjiha) {
    items.push(['Q3', '역류방지밸브·물막이판이 설치돼 있나요? 없다면 집주인 부담 설치가 가능한가요?']);
  }
...
function buildCrossSection(property, rain) {
  const safest = PROPS.reduce((current, item) => (item.thr > current.thr ? item : current));
  const alternative = property.id === safest.id ? PROPS.find((item) => item.id === 'p5') : safest;
  const ground = 150;
  ...
  const waterY = Math.min(ground, Math.max(30, ground - (rain / elevationMax / 9.6) * 95));
  ...
  return (
    <header className="brandbar">
      <div className="brandbar__l">
        <span className="logo">장마등급<b>°</b></span>
        <span className="brandbar__tag">침수 안전등급이 보이는 부동산</span>
      </div>
      <div className="brandbar__thesis">채광·주차·곰팡이 후기는 있는데, <b>이 집이 몇 mm부터 잠기는지</b>는 없었다.</div>
    </header>
  );
}
    ...
    <div className="filters">
      <div className="seg">
        {ROOM_OPTIONS.map((item) => (
          <button key={item} className={roomType === item ? 'on' : ''} onClick={() => onRoomType(item)}>{item}</button>
        ))}
      </div>
      <label className="chk">
        <input type="checkbox" checked={hideRisk} onChange={(event) => onHideRisk(event.target.checked)} />
        침수 위험 매물 숨기기
      </label>
      <div className="fdrop">
        <select aria-label="거래방식" value={dealType} onChange={(event) => onDealType(event.target.value)}>
          {DEAL_OPTIONS.map((option) => (
            <option key={option} value={option}>{option === '전체' ? '거래방식 전체' : option}</option>
          ))}
        </select>
      </div>
      <div className={`priceFilter ${priceOpen ? 'open' : ''}`}>
        <button className="priceFilter__btn" type="button" onClick={() => setPriceOpen((value) => !value)}>
          <span>가격대</span>
          <PriceValueChips dealType={dealType} priceLimits={displayPriceLimits} />
        </button>
        {priceOpen && (
          <div className="priceFilter__panel">
            {visiblePriceTypes.map((type) => (
              <PriceSlider
                ...
                onChange={(value) => setDraftPriceLimits((current) => ({ ...current, [type]: value }))}
              />
            ))}
            <div className="priceFilter__actions">
              <PriceValueChips dealType={dealType} priceLimits={draftPriceLimits} />
              <button className="priceFilter__apply" type="button" onClick={applyPriceLimits}>적용</button>
            </div>
          </div>
        )}
      </div>
      <div className="fdrop hot">
        ...
        <select aria-label="침수등급" value={minThreshold} onChange={(event) => onMinThreshold(Number(event.target.value))}>
      ...
      <button className="refresh" title="필터 초기화" type="button" onClick={onReset}>⟲</button>
    </div>
  );
}
...
function PropertyCard({ property, selected, onSelect }) {
  ...
  return (
    <button className={`card ${selected ? 'sel' : ''}`} type="button" onClick={() => onSelect(property.id)}>
      <div className="card__thumb">
        <span className="card__gtag" style={{ background: HEX[grade.g] }}>{grade.g}</span>
        <svg className="home" width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#8a97a0" strokeWidth="1.5" aria-hidden="true">
      ...
      <div className="card__b">
        ...
        <span className="gradechip" style={{ background: HEX[grade.g] }}>
          침수 {grade.g}등급 <span className="mm">· 임계점 {property.thr}mm</span>
        </span>
        <div className="card__meta">{property.agency} · <span className="chk2">확인매물 {property.checked}</span></div>
      </div>
    </button>
  );
}
      ...
      <div className="list__head">
        <div className="sort">
          {[
            ...
            <button key={value} className={sortMode === value ? 'on' : ''} onClick={() => onSort(value)}>{label}</button>
          ))}
        </div>
        <div className="list__cnt">매물 <b>{properties.length}</b></div>
      </div>
      <div className="cards">
        {properties.map((property) => (
          <PropertyCard key={property.id} property={property} selected={property.id === selectedId} onSelect={onSelect} />
        ))}
      </div>
    </aside>
  );
...
function MapPane({ rain, selectedId, selectedProperty, visiblePropertyIds, floodOn, onSelect, onToggleFlood }) {
  ...
  useEffect(() => {
    ...
    const map = L.map(mapElementRef.current, { zoomControl: true }).setView([36.3018, 127.3742], 16);
    ...
    PROPS.forEach((property) => {
      const marker = L.marker([property.lat, property.lng], { icon: markerIcon(property, 0, null) }).addTo(map);
  ...
  useEffect(() => {
    zoneLayersRef.current.forEach((layer, index) => {
      const active = Boolean(floodOn && selectedProperty && rain >= FLOOD_ZONES[index].thr);
      layer.setStyle({
  ...
  useEffect(() => {
    ...
    const center = selectedProperty ? [selectedProperty.lat, selectedProperty.lng] : [0, 0];
  ...
  useEffect(() => {
    ...
    if (property) mapRef.current.panTo([property.lat, property.lng], { animate: true });
  }, [selectedId]);

  return (
    <>
      <div ref={mapElementRef} id="map" />
      <div className="bread"><span className="pin">◉</span> 대전광역시 <i>›</i> 서구 <i>›</i> 정림동</div>
      <div className="tools">
        <button title="매물" type="button"><span className="ic">⌂</span>매물</button>
        <button className={floodOn ? 'on' : ''} title="침수 레이어" type="button" onClick={onToggleFlood}><span className="ic">≋</span>침수</button>
        <button title="학군" type="button"><span className="ic">⌘</span>학군</button>
        <button title="편의" type="button"><span className="ic">□</span>편의</button>
      </div>
    </>
  );
}
  ...
  return (
    <div className="rainpanel">
      <div className="rp__row">
        <div>
          <div className="rp__read"><span className="rp__num">{rain}</span><span className="rp__unit">mm / 시간</span></div>
          <div className="rp__desc">{description}</div>
        </div>
        <div className="rp__status">
          선택 매물 임계점<br />
          <span className={`rp__cnt ${flooded ? 'hot' : ''}`}>{property.thr}</span> <span className="rp__total">mm/h</span>
        </div>
      </div>
      <div className="rp__track">
        <div className="rp__2020">2020.7.30 정림동 78mm</div>
        <input type="range" min="0" max="115" value={rain} step="1" aria-label="시간당 강수량" onChange={(event) => onRain(Number(event.target.value))} />
        <div className="rp__marks"><span>0</span><span>30</span><span>60</span><span>90</span><span>115mm</span></div>
      </div>
    </div>
  );
}
...
function DetailDrawer({ property, rain, onClose }) {
  const [report, setReport] = useState('아래 버튼을 눌러 이 매물의 솔직한 브리핑을 받아보세요.');
  ...
  const streamReport = () => {
    ...
    setTimeout(() => {
      const timer = setInterval(() => {
        index += 1;
        setReport(`${tokens.slice(0, index).join('')}<span class="cursor"></span>`);
    ...
    <aside className="drawer open">
      <div className="dw__top">
        <div>
          ...
          <div className="p-addr">{property.addr} · {property.deal}</div>
          <span className={`p-type ${property.banjiha ? 'banjiha' : ''}`}>{property.banjiha ? '반지하' : '지상'}</span>
        </div>
        <button className="dw__x" title="닫기" type="button" onClick={onClose}>✕</button>
      </div>
      <div className="dw__body">
        <div className="hero">
          <div className="hero__thr">
            <div className="lbl">침수 임계점</div>
            <div className="val">{property.thr}<small>mm/h</small></div>
        ...
        <div className={`alert ${alertOn ? 'show' : ''}`}>
          ...
          <span>장마철 실시간 모드 · 현재 {rain}mm &gt; 임계점 {property.thr}mm — 지금 이 비라면 침수 위험</span>
        </div>

        <section className="sec">
          <div className="sec__t">단면 비교 · 같은 예산 대안</div>
          <div className="xsec">
            <svg viewBox="0 0 400 190" preserveAspectRatio="xMidYMid meet" dangerouslySetInnerHTML={{ __html: crossSection.svg }} />
            <div className="xcap"><span>{crossSection.leftCaption}</span><span>{crossSection.rightCaption}</span></div>
          </div>
        </section>
          ...
          <ul className="hist">
            {property.hist.map((history) => (
              <li key={`${history.date}-${history.txt}`}>
                <div className="hist__date">{history.date}<div className="hist__mm">{history.mm ? `${history.mm}mm/h` : '—'}</div></div>
                <div><div className={`hist__st ${history.st}`}>{history.txt}</div><div className="hist__note">{history.note}</div></div>
              </li>
            ))}
          </ul>
        </section>
        ...
        <section className="sec">
          ...
          <div className="ai">
            <div className="ai__head"><span className="ai__badge">AI</span><span>공공데이터를 사람 말로 번역합니다</span></div>
            <div className="ai__body" dangerouslySetInnerHTML={{ __html: report }} />
            <button className="btn" type="button" disabled={reporting} onClick={streamReport}>
              {reporting ? '공공데이터 분석 중…' : '이 집, 계약해도 될까요? — 리포트 생성'}
            </button>
          </div>
        </section>
        ...
        <section className="sec">
          ...
          <button className="btn" type="button" onClick={() => setCoachOpen(true)}>집 보러 갈 때 물어볼 질문 만들기</button>
          <ul className={`coach ${coachOpen ? 'show' : ''}`}>
            {coachItems(property).map(([label, text]) => (
              <li key={label}><b>{label}</b><span>{text}</span></li>
            ))}
          </ul>
        </section>

        <div className="foot">DEMO · 출처(활용 예정): <b>행안부 침수흔적도</b> · <b>국토지리정보원 DEM</b> · <b>기상청 강수량</b>. 지도 © OpenStreetMap · © CARTO. 수치는 시연용 예시.</div>
      </div>
    </aside>
  );
}
...
export default function App() {
  ...
  const visibleProperties = useMemo(() => {
    ...
    const filtered = PROPS.filter((property) => matchesFilters(property, filters));
    if (sortMode === 'price') return [...filtered].sort((a, b) => a.pval - b.pval);
    if (sortMode === 'safe') return [...filtered].sort((a, b) => b.thr - a.thr);
    if (sortMode === 'recent') return [...filtered].sort((a, b) => b.recent - a.recent);
    return filtered;
  ...
  return (
    <>
      ...
      />
      <main className="work">
        <PropertyList properties={visibleProperties} selectedId={selectedId} sortMode={sortMode} onSelect={setSelectedId} onSort={setSortMode} />
        <section className="mapwrap">
          ...
          />
          {selectedProperty && <RainPanel rain={rain} property={selectedProperty} onRain={setRain} />}
          <DetailDrawer property={selectedProperty} rain={rain} onClose={() => setSelectedId(null)} />
        </section>
      </main>
    </>
  );
